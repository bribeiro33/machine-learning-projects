...
def Rotate(deg=20):
    ...
    def _rotate(img):
        ...
        """
        random_degree = np.random.randint(-deg, deg)
        rotated_img = rotate(img, random_degree, reshape = False)
        return rotated_img
...
def Grayscale():
    ...
    def _grayscale(img):
        ...
        """
        # gray_chan = np.mean(img, axis = 2).astype(np.uint8)
        # gray_img = np.dstack((gray_chan,gray_chan,gray_chan))
        # return gray_img
        mat = img.mean(axis=2).round().astype(np.uint8)[:,:,np.newaxis]
        return np.repeat(mat, 3, axis=2)
