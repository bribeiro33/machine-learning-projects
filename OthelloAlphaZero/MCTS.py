...
class MCTS():
    ...
    def select(self, state, board):
        ...
        u = float('-inf')
        action = None
        for valid_action in self.getValidActions(state):
            u_prime = self.getConfidenceVal(state, valid_action)
            if u_prime > u:
                u = u_prime
                action = valid_action
        # End implementation
    ...
    def backpropagate(self, seq):
        # TODO: your implementation goes here
        r = 0 
        for s, action, r_prime in reversed(seq):
    ...
    def simulate(self, state, board):
        # TODO: your implementation goes here
        return self.predict(state, board)
    ...
    def search(self, initial_board):
        ...
        for _ in range(self.args.numMCTSSims):
            ...
            r = None
            while not r: 
                select_vars = self.select(s, b)
                seq.append((s, select_vars[2], select_vars[3]))
                s = select_vars[0]
                b = select_vars[1]
                r = select_vars[3]
