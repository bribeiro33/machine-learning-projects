...
class Agent:
    ...
    def h_minimax(self, depth, node, maximizingPlayer, alpha, beta):
        ...
        if node.isTerminalNode or depth==0:
        ...
        if maximizingPlayer: 
            ...
            for child in self.expandNode(node, depth): 
                curr_score, curr_move = self.h_minimax(depth - 1, child, False, alpha, beta)
                if curr_score > best_score:
                    best_score = curr_score
                    best_move = curr_move
                # if best_score >= beta: 
                #     break  #delete for no pruning
                alpha = max(alpha, best_score)
            return best_score, [node.move]+best_move
        
        else:
            best_score = self.MAX
            best_move = node.move
            for child in self.expandNode(node, depth): 
                curr_score, curr_move = self.h_minimax(depth - 1, child, True, alpha, beta)
                if curr_score < best_score:
                    best_score = curr_score
                    best_move = curr_move
                # if best_score <= alpha: 
